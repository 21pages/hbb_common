...
#[cfg(not(target_os = "windows"))]
fn should_mark_init_failed(err: &SecretStoreError) -> bool {
...
fn load_or_create_master_secretbox_key() -> SecretStoreResult<sodiumoxide::crypto::secretbox::Key> {
    ...
    if config::SecretStatus::load().is_init_failed() {
    ...
    let keybuf = match load_secret(&service, &account, MASTER_KEY_LEN) {
        ...
        Err(err) => {
            ...
            if should_mark_init_failed(&err) {
                config::SecretStatus::mark_init_failed();
            }
            return Err(err);
        }
    };
