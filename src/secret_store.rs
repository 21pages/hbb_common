...
impl From<SecretStoreError> for CachedSecretStoreError {
    fn from(err: SecretStoreError) -> Self {
        match err {
            ...
            SecretStoreError::InvalidLength { expected, actual } => {
...
impl CachedSecretStoreError {
    fn to_secret_store_error(&self) -> SecretStoreError {
        match self {
            ...
            Self::InvalidLength { expected, actual } => SecretStoreError::InvalidLength {
                expected: *expected,
                actual: *actual,
            },
...
fn master_secretbox_key() -> SecretStoreResult<&'static sodiumoxide::crypto::secretbox::Key> {
    // Cache both the first successful key load and the first initialization
    // failure so a broken secret backend does not repeatedly block every later
...
fn load_or_create_master_secretbox_key() -> SecretStoreResult<sodiumoxide::crypto::secretbox::Key> {
    ...
    let create_and_store_key = || -> SecretStoreResult<Vec<u8>> {
        log::info!(
            "Master key not found, generating new {} byte key (service: {}, account: {})",
            MASTER_KEY_LEN,
            service,
            account
        );
    ...
    let keybuf = match load_secret(&service, &account, MASTER_KEY_LEN) {
        ...
        Err(SecretStoreError::NotFound) => create_and_store_key()?,
        Err(err) => {
    ...
    Ok(sodiumoxide::crypto::secretbox::Key(
        keybuf
            .try_into()
            .map_err(|_| SecretStoreError::InvalidLength {
