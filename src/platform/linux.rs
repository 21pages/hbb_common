...
use ashpd::desktop::secret::Secret as FlatpakSecretPortal;
...
const FLATPAK_PORTAL_TIMEOUT_MS: u64 = 1_000;
...
fn retrieve_flatpak_portal_secret() -> SecretStoreResult<Vec<u8>> {
    run_flatpak_portal_task(|| async {
        ...
        let portal = FlatpakSecretPortal::new().await.map_err(|err| {
            SecretStoreError::backend(
                "failed to connect to xdg-desktop-portal secret interface",
                err,
            )
        })?;
        let (mut secret_reader, mut secret_writer) = UnixStream::pair().map_err(|err| {
            SecretStoreError::backend("failed to create xdg-desktop-portal secret buffer", err)
        })?;
        let request = portal
            .retrieve(&secret_writer.as_fd())
            .await
            .map_err(|err| {
                SecretStoreError::backend("failed to request xdg-desktop-portal secret", err)
            })?;
        request.response().map_err(|err| {
            SecretStoreError::backend("failed to retrieve xdg-desktop-portal secret", err)
        })?;
        secret_writer.shutdown().await.map_err(|err| {
            SecretStoreError::backend("failed to close xdg-desktop-portal secret buffer", err)
        })?;
...
pub fn store_secret_store_key(
    ...
    let search = ss.search_items(attrs.clone()).map_err(|err| {
        SecretStoreError::backend("failed to search Linux Secret Service items", err)
    })?;
    if !search.locked.is_empty() {
        let item_refs: Vec<&Item> = search.locked.iter().collect();
    ...
    for item in search.unlocked.iter().chain(search.locked.iter()) {
        item.delete().map_err(|err| {
            SecretStoreError::backend("failed to delete Linux Secret Service item", err)
        })?;
    }
