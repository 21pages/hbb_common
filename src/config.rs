...
use crate::{
    ...
    password_security::{
        ...
        symmetric_crypt_02_system_user, VERSION_00_UUID, VERSION_02_SYSTEM_USER,
    },
};
...
const USER_SCOPE_CONFIG_ENC_VERSION: &str = VERSION_02_SYSTEM_USER;
...
impl Config {
    ...
    fn migrate_permanent_password_to_hashed_storage(config: &mut Config) -> bool {
        ...
        if config.password.starts_with(GLOBAL_ENC_VERSION) {
            let (plain, decrypted, should_store) =
                decrypt_str_or_original(&config.password, GLOBAL_ENC_VERSION);
