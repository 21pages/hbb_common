...
impl PeerConfig {
    pub fn load(id: &str) -> PeerConfig {
        ...
        match confy::load_path(Self::path(id)) {
            Ok(config) => {
                ...
                log::info!("DDDDDDDDDDDD decrypt peer config for id: {}, password: {:?} -> {:?}, store: {}, success: {} ============", id, config.password, password, store2, succ);
                config.password = password;
                store = store || store2;
                for opt in ["rdp_password", "os-username", "os-password"] {
                    if let Some(v) = config.options.get_mut(opt) {
                        let (encrypted, succ, store2) =
                            decrypt_str_or_original(v, USER_ENC_VERSION);
                        log::info!("DDDDDDDDDDDD decrypt peer config for id: {}, opt: {}, value: {:?} -> {:?}, store: {}, success: {} ============", id, opt, v, encrypted, store2, succ);
                        *v = encrypted;
                        store = store || store2;
                    }
                }
    ...
    fn store_(&self, id: &str) {
        ...
        log::info!(
            "DDDDDDDDDDDD encrypt peer config for id: {}, password: {:?} ============",
            id,
            config.password
        );
        for opt in ["rdp_password", "os-username", "os-password"] {
            if let Some(v) = config.options.get_mut(opt) {
                *v = encrypt_str_or_original(v, USER_ENC_VERSION, ENCRYPT_MAX_LEN);
                log::info!("DDDDDDDDDDDD encrypt peer config for id: {}, opt: {}, value: {:?} ============", id, opt, v);
            }
        }
...
impl LocalConfig {
    ...
    fn load() -> LocalConfig {
        ...
        if let Some(access_token) = config.options.get_mut(Self::ACCESS_TOKEN_KEY) {
            let (token, succ, store2) = decrypt_str_or_original(access_token, USER_ENC_VERSION);
            *access_token = token;
            store |= store2;
            log::info!(
                    "DDDDDDDDDDDDDDDDD Access token decrypted from local config: {}, success: {}, need_store: {}",
                    access_token, succ, store2
                );
        }
        if store {
            config.store();
    ...
    fn store(&self) {
        ...
        if let Some(access_token) = config.options.get_mut(Self::ACCESS_TOKEN_KEY) {
            *access_token =
            ...
            log::info!(
                "DDDDDDDDDDDDDDDDD Access token encrypted in local config: {}",
                *access_token
            );
        }
...
impl Ab {
    ...
    pub fn store(json: String) {
        if let Ok(mut file) = std::fs::File::create(Self::path()) {
            ...
            if encrypted != data {
                log::info!("================ Ab data encrypted successfully");
                file.write_all(&encrypted).ok();
            } else {
                log::error!("XXXXXXXXXXXXXXXXX Ab data not encrypted");
            }
        };
    }
    ...
    pub fn load() -> Ab {
        if let Ok(mut file) = std::fs::File::open(Self::path()) {
            ...
            if file.read_to_end(&mut data).is_ok() {
                ...
                if success {
                    log::info!("================ Group data decrypted successfully");
                    let data = decompress(&data);
                    if let Ok(ab) = serde_json::from_str::<Ab>(&String::from_utf8_lossy(&data)) {
                        log::info!("================ Group data loaded successfully");
                        return ab;
                    } else {
                        log::error!("xxxxxxxxxxxxxxxxxx Failed to parse group data");
                    }
                } else {
                    log::error!("xxxxxxxxxxxxxxxxxx Failed to decrypt group data");
                }
            }
        };
        Self::remove();
...
impl Group {
    ...
    pub fn store(json: String) {
        if let Ok(mut file) = std::fs::File::create(Self::path()) {
            ...
            let encrypted = encrypt_vec_or_original(&data, USER_ENC_VERSION, max_len);
            if encrypted != data {
                log::info!("================ Group data encrypted successfully");
                file.write_all(&encrypted).ok();
            } else {
                log::error!("XXXXXXXXXXXXXXXXX Group data not encrypted");
            }
        };
    }
...
mod tests {
    ...
    lazy_static::lazy_static! {
        static ref TEST_APP_LOCK: Mutex<()> = Mutex::new(());
    }

    struct AppNameGuard(String);

    impl AppNameGuard {
        fn new(app_name: &str) -> Self {
            let mut current = APP_NAME.write().unwrap();
            let previous = current.clone();
            *current = app_name.to_owned();
            Self(previous)
        }
    }
    ...
    impl Drop for AppNameGuard {
        fn drop(&mut self) {
            *APP_NAME.write().unwrap() = std::mem::take(&mut self.0);
        }
    }
    ...
    fn test_local_config_encrypts_access_token_only() {
        ...
        assert_eq!(stored["remote_id"].as_str(), Some("123456"));
        assert_eq!(
            stored["options"]["user_info"].as_str(),
            Some("{\"name\":\"plain\"}")
        );
        let access_token = stored["options"]["access_token"].as_str().unwrap();
        assert!(access_token.starts_with("02"));
        assert_ne!(access_token, "secret-token");
