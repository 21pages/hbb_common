...
use crate::{
    ...
    log,
    password_security::{
        decrypt_str_or_original, decrypt_user_sync_file, decrypt_vec_or_original,
        encrypt_str_or_original, encrypt_user_sync_file, encrypt_vec_or_original,
        VERSION_00_UUID, VERSION_02_USER,
    },
};
...
impl Ab {
    ...
    pub fn store(json: String) {
        if let Ok(mut file) = std::fs::File::create(Self::path()) {
            ...
            if let Some(data) = encrypt_user_sync_file(&data, max_len) {
    ...
    pub fn load() -> Ab {
        if let Ok(mut file) = std::fs::File::open(Self::path()) {
            ...
            if file.read_to_end(&mut data).is_ok() {
                if let Some(data) = decrypt_user_sync_file(&data) {
