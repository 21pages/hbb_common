...
use crate::{
    ...
    password_security::{
        ...
        encrypt_vec_or_original, symmetric_crypt_02_user, VERSION_00_UUID, VERSION_02_USER,
    },
};
...
impl Ab {
    ...
    pub fn store(json: String) {
        if let Ok(mut file) = std::fs::File::create(Self::path()) {
            ...
            if let Ok(data) = symmetric_crypt_02_user(&data, true) {
    ...
    pub fn load() -> Ab {
        if let Ok(mut file) = std::fs::File::open(Self::path()) {
            ...
            if file.read_to_end(&mut data).is_ok() {
                if let Ok(data) = symmetric_crypt_02_user(&data, false) {
